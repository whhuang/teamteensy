...
*/
/**************************************************************************/
boolean Adafruit_VL6180X::begin(uint8_t whichOne) {
...
uint8_t Adafruit_VL6180X::read8(uint16_t address)
{
  ...
  whichWire.beginTransmission(_i2caddr);
  whichWire.write(address>>8);
  whichWire.write(address);
  whichWire.endTransmission();

  whichWire.requestFrom(_i2caddr, (uint8_t)1);
  uint8_t r = whichWire.read();
...
uint16_t Adafruit_VL6180X::read16(uint16_t address)
{
  ...
  whichWire.requestFrom(_i2caddr, (uint8_t)2);
  while(!whichWire.available());
  data = whichWire.read();
  data <<= 8;
  while(!whichWire.available());
  data |= whichWire.read();
